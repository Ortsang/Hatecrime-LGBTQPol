...
drop if year_earliest<=2009 | leader_pre_all>0
drop if state == "DELAWARE" // Delaware has Mark at Aguirre Rehoboth Beach City Commission (2003-?)
...
bysort lgbt: hdidregress aipw (_menthlth_d c.hc_crntyr c.hc_prvsyr ///
	...
	c.enroll i.estate#c.year i.LSSM) ///
...
hdidregress aipw (_menthlth c.hc_crntyr c.hc_prvsyr ///
	...
	i.acaexpan c.pop c.agencies c.highschool c.malefemaleratio c.unemployment ///
	i.election2016##c.trumprate /// 
	c.enroll i.estate#c.year /* i.month i.year */ i.LSSM) ///
	(post leader_all), group(estate) time(year)
